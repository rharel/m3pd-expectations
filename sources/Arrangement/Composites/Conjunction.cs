...
namespace rharel.M3PD.Expectations.Arrangement
{
    ...
    public sealed class Conjunction: Node
    {
        ...
        /// </exception>
        public Conjunction(string id, IEnumerable<Node> children)
            : base(id, children)
        {
            if (Children.Count < 2)
            {
        ...
        {
            bool all_children_are_satisfied = true;
            foreach (var child in Children)
            {
                ...
                {
                    case Resolution.Failure: return Resolution.Failure;
                    case Resolution.Pending:
                    {
                        all_children_are_satisfied = false;
                        break;
                    }
                }
            }
            return all_children_are_satisfied ?
